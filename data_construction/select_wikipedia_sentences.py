"""
Extract sentences containing specified entities from English Wikipedia
articles.

This script requires:
    - output from WikiExtractor (https://github.com/attardi/wikiextractor)
    - target entities (a list of Wikipedia pageid)

This script uses wptools (https://github.com/siznax/wptools) to get entity
metadata. This is the speed bottleneck, so you use the cached metadata (e.g.,
run wptools on the target entities separately)
"""
    ...
    def get_sentences(
        ...
        for doc in self.docs[:top_paragraphs]:
            ...
            for sent in doc.sents:
                if paraid == 0:
                    # Skip all sentences in the first paragraph.
                    # We might want to control sentences as well.
                    continue
                else:
                    for ent in aliases:
                        try:
...
def main():
    ...
    # Use cached wiki page metadata (this is much much faster).
    if args.cached_wiki_pages:
        with open(args.cached_wiki_pages) as f:
            cached_wiki_pages = [json.loads(l.strip()) for l in f]
            cached_wiki_pages = {cw['pageid']: cw for cw in cached_wiki_pages}
    else:
        cached_wiki_pages = None

    # Create save dir if it doesn't exist
    if not os.path.exists(args.save_to):
        save_dir = '/'.join(args.save_to.split('/')[:-1])
